...
@task
def storage_task(cfg: StorageConfig, transcripts: list[dict]):
...
def orchestrator():
    ...
    transcripts = transcription_task(
        TranscriptionConfig(ws_url=settings.whisper_url),
        frames,
    )
    stored = storage_task(
        StorageConfig(db_url=settings.db_url, session_id=settings.meeting_id),
        transcripts,
    )
    chunks = chunking_task(ChunkingConfig(), stored)
    summaries = summarization_task(
        SummarizationConfig(
